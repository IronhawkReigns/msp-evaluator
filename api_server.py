...
async def ask_question(request: Request):
    ...
    try:
        ...
        prompt = (
            ...
            f"결코 없는 정보를 추론하거나 일반적인 기대를 기반으로 답하지 마세요. 명확한 정보가 없는 경우, 해당 회사를 제외하십시오.\n"
            f"대개 사용자의 질문에 관련도가 매우 높은 데이터가 있습니다. 이 데이터를 우선으로 기반하여 답하세요."
            f"각 Q&A 항목의 score도 참고해 주세요. 높은 score는 강한 관련성을 의미합니다.\n\n"
            f"응답은 다음 형식으로 작성해주세요:\n"
            f"회사명을 굵게 표시하고, 각 회사를 별도의 단락으로 설명해주세요. 예:\n\n"
            f"**디딤365**\n"
            f"- 선정 이유: 데이터 처리 및 분석 역량이 뛰어나며 다양한 수상 경력을 보유하고 있어 신뢰성과 전문성이 높음\n\n"
            f"**로민**\n"
            f"- 선정 이유: OCR 특화 기술 및 현업 기반 평가 경험이 풍부하며 실적과 협업 구조가 우수함\n\n"
            f"**기타 회사**\n"
