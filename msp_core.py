...
def generate_answer_pdf(answer: str, evidence: list, filename: str = "msp_report.pdf") -> str:
    class PDF(FPDF):
        def header(self):
            self.set_font("Arial", "B", 12)
            self.cell(0, 10, "MSP 평가 리포트", 0, 1, "C")
    ...
    pdf.chapter_title("1. AI 응답")
    pdf.chapter_body(answer)

    if evidence:
        pdf.chapter_title("2. 근거 데이터")
        for idx, item in enumerate(evidence, 1):
            score = item.get("score", "-")
            question = item.get("question", "")
            response = item.get("answer", "")
            msp = item.get("msp_name", "알 수 없음")
